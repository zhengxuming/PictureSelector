...
public final class LocalMediaPageLoader extends IBridgeMediaLoader {
    ...
    private String getSelectionArgsForAllMediaCondition(String timeCondition, String sizeCondition, String queryMimeTypeOptions) {
        ...
        Log.i("frag","getSelectionAll------->"+ stringBuilder);
        if (isWithAllQuery()) {
    ...
    /**
     * Audio and Video mode conditions
     *
     */
    private String getSelectionArgsForAVMediaCondition(String durationCondition, String queryMimeTypeOptions) {

        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append(MediaStore.Files.FileColumns.MEDIA_TYPE).append("=?")
                .append(queryMimeTypeOptions).append(" OR ")
                .append(MediaStore.Files.FileColumns.MEDIA_TYPE).append("=? AND ").append(durationCondition);

        Log.i("frag","getSelectionAudioAndVideo------->"+ stringBuilder);
        if (isWithAllQuery()) {
            return stringBuilder.toString();
        } else {
            return stringBuilder.append(")").append(GROUP_BY_BUCKET_Id).toString();
    ...
    public void loadPageMediaData(long bucketId, int page, int pageSize, OnQueryDataResultListener<LocalMedia> listener) {
        ...
        Log.i("frag","loadPageMediaData----bucketId-->"+bucketId);
        ...
        PictureThreadUtils.executeByIo(new PictureThreadUtils.SimpleTask<MediaData>() {
            ...
            public MediaData doInBackground() {
                ...
                try {
                    ...
                    e.printStackTrace();
                    Log.i("frag", "loadMedia Page Data Error: " + e.getMessage());
            ...
            public void onSuccess(MediaData result) {
                Log.i("frag","loadPageMediaData----result-->"+result.data.size());
                ...
                try {
                    if (data != null) {
                        ...
                        if (count > 0) {
                            ...
                            Log.i("frag","query----->"+mediaFolders.size());
                            return mediaFolders;
                        }
                    }
                } catch (Exception e) {
    ...
    private String getPageSelection(long bucketId) {
        ...
        switch (getConfig().chooseMode) {
            ...
            case SelectMimeType.TYPE_AV:
                //  Gets the video or audio
                return getPageSelectionArgsForAVMediaCondition(bucketId, queryMimeCondition, durationCondition, sizeCondition);
        }
        return null;
    }
    ...
    private static String getPageSelectionArgsForAllMediaCondition(long bucketId, String queryMimeCondition, String durationCondition, String sizeCondition) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("(").append(MediaStore.Files.FileColumns.MEDIA_TYPE).append("=?").append(queryMimeCondition).append(" OR ").append(MediaStore.Files.FileColumns.MEDIA_TYPE).append("=? AND ").append(durationCondition).append(") AND ");
    ...
    private static String getPageSelectionArgsForAVMediaCondition(long bucketId, String queryMimeCondition, String durationCondition, String sizeCondition) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("(").append(MediaStore.Files.FileColumns.MEDIA_TYPE).append("=?").append(queryMimeCondition).append(" OR ").append(MediaStore.Files.FileColumns.MEDIA_TYPE).append("=? AND ").append(durationCondition).append(") AND ");
        if (bucketId == PictureConfig.ALL) {
            return stringBuilder.append(sizeCondition).toString();
    ...
    protected String getSelection() {
        ...
        String queryMimeCondition = getQueryMimeCondition();
        Log.i("frag", "LocalMediaPageLoader-----getSelection------>" + queryMimeCondition);
        switch (getConfig().chooseMode) {
            case SelectMimeType.TYPE_ALL:
            ...
            case SelectMimeType.TYPE_AV:
                // Access to the audio
                return getSelectionArgsForAVMediaCondition(durationCondition, queryMimeCondition);
        }
        return null;
    }
